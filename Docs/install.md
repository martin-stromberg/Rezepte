...
Diese Anleitung beschreibt das manuelle Deployment der Anwendung auf einem Linux-Server mit systemd. Die Anwendung zielt auf `net10.0` und wird typischerweise fÃ¼r `linux-x64` verÃ¶ffentlicht.

## Runtime auf dem Server prÃ¼fen

Bei einem framework-abhÃ¤ngigen Publish (`--self-contained false`) mÃ¼ssen die passenden .NET-10-Shared-Frameworks auf dem Zielserver installiert sein. PrÃ¼fe die installierte Runtime vor dem Ausrollen:

```bash
dotnet --info
```

In der Ausgabe mÃ¼ssen beide Shared Frameworks in einer passenden .NET-10-Version vorhanden sein:

- `Microsoft.NETCore.App`
- `Microsoft.AspNetCore.App`

Framework-Assemblies wie `System.Runtime.Serialization.Primitives.dll` mÃ¼ssen bei einem framework-abhÃ¤ngigen Publish nicht im Publish-Verzeichnis liegen. Sie werden aus dem installierten .NET-Shared-Framework des Servers geladen. Fehlt dort eine passende .NET-10-Runtime, kann die Anwendung beim Rendern von Blazor-Komponenten mit `System.IO.FileNotFoundException` abbrechen.

## Publish erzeugen

Verwende framework-abhÃ¤ngiges Deployment, wenn der Server die passenden .NET-10-Shared-Frameworks verlaesslich bereitstellt:
...
Erstelle `/etc/systemd/system/rezepte.service` fÃ¼r ein framework-abhÃ¤ngiges Deployment:
...
`WorkingDirectory=/var/www/rezepte` muss dabei erhalten bleiben, damit relative Pfade fÃ¼r Datenbank, Logs und statische Dateien zum Deployment-Verzeichnis passen.
