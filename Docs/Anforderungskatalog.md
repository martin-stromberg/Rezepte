...
| FR-025    | ✅ Erledigt   | Bereinigung alter Exporte: ZIP-Archive im `exports`-Ordner werden spätestens einen Tag nach Erstellung automatisch gelöscht | `ExportCleanupHostedService` prüft minütlich, ob die tägliche Bereinigung fällig ist; `ExportCleanupService` löscht abgelaufene Dateien inkl. `UserExportFiles`-Einträgen und verwaiste ZIPs. Uhrzeit konfigurierbar unter "Sicherung" (Default 03:00, Serverzeit; `AppSetting` `ExportCleanup:Time`); letzter Lauf in `ExportCleanup:LastRunAt`, verpasste Läufe werden beim nächsten Start nachgeholt. API: `GET/PUT /api/admin/exports/cleanup`, `POST /api/admin/exports/cleanup/run`. Siehe Issue #110. |
